jQuery(document).ready(function ($) {
    ...
    for (let index = 0; index < _block_styleExtra.length; index++) {
        //console.log(_block_styleExtra[index].id);
    ...
    var _block_editorScript = null;
    let _block_scriptExtra = document.getElementsByClassName('wb-codemirror-js');
    for (let index = 0; index < _block_scriptExtra.length; index++) {
        //console.log(_block_scriptExtra[index].id);
        let editor = wp.codeEditor.initialize(_block_scriptExtra[index], editorSettings);
        if (_block_scriptExtra[index].classList.contains('wb-asset-editorscript-js')) {
            _block_editorScript = editor;
        }
    }
    ...
    */
    ...
    var attr_conditions = jQuery('#_block_attributes_condition');
    ...
    function toggle_attributes_condition() {
        //console.log(jQuery('#_block_editorScript_file').val());
        if (!jQuery('#_block_editorScript_file').val()
             || jQuery('#_block_editorScript_file').val().startsWith('/* generated by wizard-blocks')) {
             // enable conditions
         } else {
             // disable conditions
         }
    ...
    function get_field_condition(key) {
        if (attr_conditions.val()) {
            let tmp = JSON.parse(attr_conditions.val());
            if (tmp[key]) {
                return tmp[key];
            }
        }
    ...
    function update_block_attributes() {
        ...
        attr_editor.find('.repeat_attr').each(function (index, row) {
            ...
            if (key) {
                ...
                if (row.find('.condition').val()) {
                    conditions[key] = row.find('.condition').val();
                }
            }
        });
        //console.log(attributes);
        ...
        let cond_json = JSON.stringify(conditions, null, 4);
        //console.log(attr_json);
        attr_conditions.val(cond_json);
