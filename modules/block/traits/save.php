...
trait Save {
    ...
    public function meta_fields_save_meta_box_data($post_id, $post, $update) {
            ...
            if ($attributes == NULL) {
                ...
                foreach($attributes as $aid => $attribute) {
                    if (!empty($attribute['component'])) {
                        if ($attribute['component'] == 'InnerBlocks') {
                            $attributes[$aid]['position'] = "block";
                        }
                    }
                    if (empty($attribute['type'])) {
                        ...
                        if (!empty($attribute['component'])) {
                            switch ($attribute['component']) {
                                ...
                                case 'FontSizePicker':
                                case 'AnglePickerControl':
                                    $attributes[$aid]['type'] = 'integer';
                                    break;
                                
                                case 'FocalPointPicker':
                                case 'BoxControl':
                                    $attributes[$aid]['type'] = 'object';
                                    break;
                ...
                foreach ($attributes as $akey => $attribute) {
                    if (!empty($_POST['_block_example'][$akey])) {
                        ...
                        switch($attribute['type']) {
                            ...
                            case 'array':
                            case 'object':
                                $example['attributes'][$akey] = json_decode(wp_unslash($value), true);
                                break;
                            case 'string':
                            default:
        ...
        foreach (self::$assets as $asset => $type) {
            ...
            if (!empty($post_data['_block_' . $asset.'_file'])) {
                ...
                foreach (self::$assets_alias as $ass => $alias) {
                    list($asst,$ext) = explode('.', $ass, 2);
                    //var_dump($ass);var_dump($asst);
                    if ($asset == $asst) {
                        if (empty($post_data['_block_' . $asset.'_file'][$ass]) 
                                && !empty($post_data['_block_' . $asst.'_file'][$alias])) {
                            //var_dump($post_data['_block_' . $asset.'_file'][$alias]); die();
                            $post_data['_block_' . $asset.'_file'][$ass] = $post_data['_block_' . $asset.'_file'][$alias];
                            unset($post_data['_block_' . $asset.'_file'][$alias]);
                            //var_dump($path);
                            $file = basename($path);
                            $path = str_replace($file, $ass, $path);
                            //var_dump($path); die();
                        }
                    }
                }
            ...
            if (!empty($post_data['_block_' . $asset.'_file'])) {
                switch ($asset) {
                    case 'render':
                        $code = $post_data['_block_' . $asset.'_file'];
                        break;
                    default: // get default file
                        $code = $post_data['_block_' . $asset.'_file'][$asset.'.'.$type];
                }
            ...
            if ($asset == 'editorScript') {
                ...
                /*if (!empty($json_old[$asset])) {
                    ...
                    foreach ($editorScripts as $editorScript) {
                        ...
                        if ($editorScriptCode && str_contains($editorScriptCode, 'registerBlockType')) {
                            $registered = true;
                        }
                    }
                }*/
                if (!$registered) {
                    if (!$code || strpos($code, 'generated by '.$this->get_plugin_textdomain()) !== false) {
                        if (empty($json['supports']['autoRegister'])) { // added in WP 7.0: auto register Controls via PHP
        ...
        // SET ASSETS IN JSON
        foreach (self::$assets as $asset => $type) {
            if (!empty($post_data['_block_' . $asset])) {
                $_block_asset = sanitize_text_field(wp_unslash($post_data['_block_' . $asset]));
                ...
                foreach ($files as $file) {
                    ...
                    if (substr($file, 0, 5) != 'file:') {
                        ...
                        if ($file != $asset_file && $file != $asset_min) {
                            ...
                            $code = '';
                            if (!empty($post_data['_block_' . $asset.'_file'][$file_name])) {
                                $code = $post_data['_block_' . $asset.'_file'][$file_name];
                                $code = wp_unslash($code);                               
                                //if (file_exists($path)) {
                                    //file_put_contents($path, $code);
                                    $this->get_filesystem()->put_contents($path, $code);
                            ...
                            if ($ass = array_search($file_name, self::$assets_alias)) {
                                // duplicate?!
                                //var_dump($code); var_dump($post_data['_block_' . $asset.'_file'][$ass]);
                                if ($code == wp_unslash($post_data['_block_' . $asset.'_file'][$ass])) {
                                    //var_dump($ass);var_dump($file_name);var_dump($path); die();
                                    $this->get_filesystem()->delete($path);
                                    $path_min = str_replace('.'.$type,$min.$type,$path);
                                    //var_dump($path_min); die();
                                    $this->get_filesystem()->delete($path_min);
                                    $file = 'file:./'.$ass;
                                    //var_dump($file);
                                }
                    ...
                    if ($file) {
                        ...
                        // if asset and asset.min, remove asset
                        $file_min = str_replace('.'.$type, $min.$type, $file);
                        //var_dump($file_min);
                        if (in_array($file_min, $json[$asset])) {
                            // remove last and use only min
                            array_pop($json[$asset]);
                        }
