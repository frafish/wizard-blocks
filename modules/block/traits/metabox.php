...
// register meta box
    public function meta_fields_add_meta_box() {
        ...
        if ($this->is_block_edit()) {
            ...
            add_action('post_submitbox_start', function ($post) {
                echo '<div id="export-action" style="float: left; margin-right: 5px;"><a class="button button-secondary button-large" target="_blank" href="' . esc_url($this->get_action_url('action=download&block=' . $post->post_name)) . '">' . (esc_html__('Export', 'wizard-blocks')) . '</a></div>';
            });
        }
    }
    ...
    public function meta_fields_build_render_callback($post, $metabox) {
        ...
        if ($post) {
            ...
            $render_file = $this->get_ensure_blocks_dir($post->post_name, $json['textdomain']) . $file;
