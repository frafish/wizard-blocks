...
// register meta box
    ...
    public function meta_fields_build_meta_box_side_callback($post, $metabox) {
        ...
        <?php if (!empty($json['name'])) { ?>
            ...
            printf(esc_attr__('Used %s times in this site', 'wizard-blocks'), esc_attr($block_usage_count));
            ?></p>           
                <?php if ($block_usage_count) { ?>
                <div class="block-usage-posts-list"><ul>
                        <?php foreach ($block_usage['posts'] as $post_id) { ?>
    ...
    public function meta_fields_build_context_side_callback($post, $metabox) {
        //wp_nonce_field('meta_fields_save_meta_box_side_data', 'meta_fields_meta_box_nonce');
        $json = $post ? $this->get_block_json($post->post_name) : [];
        ?>
    ...
    public function meta_fields_build_supports_side_callback($post, $metabox) {
        //wp_nonce_field('meta_fields_save_meta_box_side_data', 'meta_fields_meta_box_nonce');
        ...
        <h4><label id="supports_custom" for="_block_supports_custom"><b><?php esc_attr_e('Supports custom values', 'wizard-blocks'); ?></b></label></h4>
    ...
    public function meta_fields_build_extra_callback($post, $metabox) {
        //wp_nonce_field('meta_fields_save_meta_box_side_data', 'meta_fields_meta_box_nonce');
        ...
        <?php if ($post && !empty($block['attributes']) && (count($block['attributes']) > 1 || !isset($block['attributes']['preview']))) { ?>
            <div class="_block_example_values">
                ...
                <table class="widefat striped">
                    ...
                    <tbody>
                        <?php
                        foreach ($block['attributes'] as $akey => $attr) {
